...
/* Drop in replacement for pthread_mutex_timedlock_winpr
 */
#if !defined(HAVE_PTHREAD_MUTEX_TIMEDLOCK)
#include <pthread.h>

static int pthread_mutex_timedlock_winpr(pthread_mutex_t* mutex, const struct timespec* timeout)
{
...
DWORD WaitForSingleObject(HANDLE hHandle, DWORD dwMilliseconds)
{
	...
	else if (Type == HANDLE_TYPE_MUTEX)
	{
		...
		if (dwMilliseconds != INFINITE)
		{
			...
			status = pthread_mutex_timedlock_winpr(&mutex->mutex, &timeout);
